...
#define BLOCK_Y 1
#define BLOCK_Z 1
#define INPUT_SIZE GRID_X * GRID_Y * GRID_Z * BLOCK_X * BLOCK_Y * BLOCK_Z
...
int initializeArray(int **arr, int size, bool fillRandom)
{
	*arr = (int*)malloc(size * sizeof(int));
	//cudaMallocHost((void**)arr, size * sizeof(int));
	if(*arr == NULL) {
		return 1;
	}
...
int initializeCharArray(char **arr, int size, bool fillRandom)
{
	*arr = (char*)malloc(size * sizeof(char));
	//cudaMallocHost((void**)arr, size * sizeof(char));
	if(*arr == NULL) {
		return 1;
	}
...
cudaError_t checkAvailableMemory(int requiredMemory)
{
...
int main()
{
	...
	//cudaFreeHost(input);
	//cudaFreeHost(prefix);
	//cudaFreeHost(outText);
	//cudaFreeHost(outAmount);
...
cudaError_t runLengthEncoding(char **outText, int **outAmount, int **temp, char **in, unsigned int size, int *outSize)
{
	...
	float transferTime = 0, gpuTime = 0;
	...
	if(checkAvailableMemory(size) != cudaSuccess) {
		goto Error;
	}

    // Allocate GPU buffers for three vectors (two input, one output)    .
    if(handleError(cudaStatus = cudaMalloc((void**)&dev_in, size * sizeof(char)), "cudaMalloc dev_in failed")) {
		goto Error;
	}
	...
	if(stopTimer(&start, &stop, "memcpy", &transferTime) != cudaSuccess) { goto Error; }
	if(startTimer(&start, &stop) != cudaSuccess) { goto Error; }
	...
	if(stopTimer(&start, &stop, "setFlags", &gpuTime) != cudaSuccess) { goto Error; }
	if(startTimer(&start, &stop) != cudaSuccess) { goto Error; }
	...
	try {
		...
		fprintf(stderr, "thrust inclusive_scan error: %s\n", ex.what());
		goto Error;
	}
	
	if(stopTimer(&start, &stop, "scan", &gpuTime) != cudaSuccess) { goto Error; }
	if(startTimer(&start, &stop) != cudaSuccess) { goto Error; }
	...
	if(stopTimer(&start, &stop, "memcpy", &gpuTime) != cudaSuccess) { goto Error; }
	if(startTimer(&start, &stop) != cudaSuccess) { goto Error; }
	...
	*outSize = outputSize;
	...
	if(stopTimer(&start, &stop, "encode", &gpuTime) != cudaSuccess) { goto Error; }
	if(startTimer(&start, &stop) != cudaSuccess) { goto Error; }
	...
	if(stopTimer(&start, &stop, "adj diff", &gpuTime) != cudaSuccess) { goto Error; }
	if(startTimer(&start, &stop) != cudaSuccess) { goto Error; }
	...
	printf("Gpu time: %f, Memory alloc/copy time: %f\n", gpuTime, transferTime);
	printf("Total runtime: %f\n", transferTime + gpuTime);
